...
function buildTimeExpression(columns) {
  ...
  return expressions.length > 0 ? `COALESCE(${expressions.join(", ")}, 0)` : "0";
}
...
export async function ensureWorkspaceRoot(codexHome, targetCwd, options = {}) {
  const filePath = globalStatePath(codexHome);
  const backupPath = globalStateBackupPath(codexHome);
  let originalText;
  try {
  ...
  const state = JSON.parse(originalText);
  const desktopTarget = toDesktopWorkspacePath(targetCwd);
  const targetComparable = normalizeComparablePath(desktopTarget);
  const existingSavedRoots = toPathArray(state["electron-saved-workspace-roots"]).map(toDesktopWorkspacePath);
  const existingProjectOrder = toPathArray(state["project-order"]).map(toDesktopWorkspacePath);
  const hasSavedRoot = existingSavedRoots.some((entry) => normalizeComparablePath(entry) === targetComparable);
  const hasProjectOrderRoot = existingProjectOrder.some((entry) => normalizeComparablePath(entry) === targetComparable);
  const nextSavedRoots = hasSavedRoot ? existingSavedRoots : dedupePaths([desktopTarget, ...existingSavedRoots]);
  const nextProjectOrder = hasProjectOrderRoot ? existingProjectOrder : dedupePaths([desktopTarget, ...existingProjectOrder]);
  const updated = !arraysEqual(existingSavedRoots, nextSavedRoots) || !arraysEqual(existingProjectOrder, nextProjectOrder);
