...
export async function updateSqliteThreadCwd(codexHome, threadId, targetCwd, options = {}) {
  const dbPath = stateDbPath(codexHome);
  try {
    await fs.access(dbPath);
  } catch {
  ...
  try {
    ...
    if (!tableHasColumn(db, "threads", "cwd")) {
      return {
        ...
        matchedRows: 0,
        changedRows: 0,
        cwdColumnPresent: false
      };
    }

    const matchedRows = db.prepare("SELECT COUNT(*) AS count FROM threads WHERE id = ?").get(threadId)?.count ?? 0;
    if (options.dryRun) {
      const changedRows = db
        .prepare("SELECT COUNT(*) AS count FROM threads WHERE id = ? AND COALESCE(cwd, '') <> ?")
        .get(threadId, targetCwd)?.count ?? 0;
      return {
        databasePresent: true,
        cwdColumnPresent: true,
        matchedRows,
        changedRows
      };
    }
    ...
    transactionOpen = true;
    const result = db
      .prepare("UPDATE threads SET cwd = ? WHERE id = ? AND COALESCE(cwd, '') <> ?")
      .run(targetCwd, threadId, targetCwd);
    db.exec("COMMIT");
    transactionOpen = false;
    return {
      databasePresent: true,
    ...
    throw wrapSqliteMalformedError(
      wrapSqliteBusyError(error, "update session cwd metadata"),
      "update session cwd metadata"
    );
  } finally {
    db?.close();
  }
