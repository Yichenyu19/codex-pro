...
export async function collectSessionCwdChanges(codexHome, threadId, targetCwd) {
  const normalizedThreadId = typeof threadId === "string" ? threadId.trim() : "";
  const normalizedTargetCwd = toDesktopWorkspacePath(typeof targetCwd === "string" ? targetCwd.trim() : "");
  if (!normalizedThreadId) {
    throw new Error("Missing thread id for session move.");
  }
  if (!normalizedTargetCwd) {
    throw new Error("Missing target workspace path for session move.");
  }
  ...
  for (const dirName of SESSION_DIRS) {
    ...
    for (const rolloutPath of rolloutPaths) {
      ...
      const parsed = parseSessionMetaRecord(record.firstLine);
      if (parsed?.payload?.id !== normalizedThreadId) {
        continue;
      }

      const originalCwd = typeof parsed.payload.cwd === "string"
        ? toDesktopWorkspacePath(parsed.payload.cwd)
        : "";
      if (originalCwd === normalizedTargetCwd) {
        changes.push({
          path: rolloutPath,
      ...
      const snapshot = await getFileSnapshot(rolloutPath);
      parsed.payload.cwd = normalizedTargetCwd;
      changes.push({
        path: rolloutPath,
        threadId: normalizedThreadId,
        directory: dirName,
