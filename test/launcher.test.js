...
test("installWindowsLauncher creates cmd and vbs launchers", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "codex-pro-launcher-"));
  ...
  assert.match(cmdText, /codex-pro sync --codex-home "C:\\Users\\Example User\\.codex"/);
  assert.match(vbsText, /Synchronization finished\./);
  assert.match(vbsText, /Codex Pro/);
  assert.match(vbsText, /codex-pro sync --codex-home ""C:\\Users\\Example User\\.codex""/);
});
